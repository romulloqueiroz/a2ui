...
describe('Example: Login Form (basic) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample({name: 'Login Form (basic)', version: Version.V0_8});
  ...
  it('should dispatch login action on button click', async () => {
    ...
    expect(loggedAction.name).toBe('login');
  });

  it('should dispatch signup action on button click', async () => {
    const component = fixture.componentInstance;
    ...
    expect(component.eventsLog.length).toBeGreaterThan(0);
    const loggedAction = component.eventsLog[0].action;
    expect(loggedAction.name).toBe('signup');
  });
});
