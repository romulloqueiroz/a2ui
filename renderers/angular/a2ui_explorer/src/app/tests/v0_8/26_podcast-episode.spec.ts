...
describe('Example: Podcast Episode (basic) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample({name: 'Podcast Episode (basic)', version: Version.V0_8});
    textContent = getCanvas().textContent;
  });

  it('should render expected text content', async () => {
    expect(textContent).toContain('Play Episode');
    ...
    expect(textContent).toContain('Dec 15, 2024');
    expect(textContent).toContain('How AI is transforming the way we design and build products.');
  });

  it('should dispatch play action on button click', async () => {
    ...
    expect(loggedAction.name).toBe('play');
  });
});
