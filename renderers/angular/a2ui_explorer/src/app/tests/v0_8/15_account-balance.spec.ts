...
describe('Example: Account Balance (basic) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample({name: 'Account Balance (basic)', version: Version.V0_8});
  ...
  it('should dispatch transfer action on button click', async () => {
    ...
    expect(loggedAction.name).toBe('transfer');
  });

  it('should dispatch pay_bill action on button click', async () => {
    const component = fixture.componentInstance;
    ...
    expect(component.eventsLog.length).toBeGreaterThan(0);
    const loggedAction = component.eventsLog[0].action;
    expect(loggedAction.name).toBe('pay_bill');
  });
});
